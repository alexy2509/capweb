...
Merci par avance.`},f=new URLSearchParams(window.location.search),i=(f.get("service")||"").toLowerCase(),c=(f.get("pack")||"").toLowerCase();if(i){const o=document.getElementById("project-type");if(o){const r=Array.from(o.options).some(e=>e.value===i);o.value=r?i:"autre"}const n=document.getElementById("message");if(n&&!n.value.trim()){const r=c?`${i}-${c}`:i,e=d[r]||d[i];e&&(n.value=e)}const s=document.getElementById("pack");s&&c&&(s.value=c)}const g=8e3;let v=0;const b=Date.now(),y=1500,t=(o,n)=>o.replace(/[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F]/g,"").trim().slice(0,n);a.addEventListener("submit",async o=>{if(o.preventDefault(),(document.getElementById("website")?.value||"").trim()!==""){a.reset(),m.hidden=!1;return}const s=Date.now();if(s-b<y||s-v<g){alert("Merci de patienter quelques secondes avant de réessayer.");return}if(!a.checkValidity()){a.reportValidity();return}u.disabled=!0,p.hidden=!0,l.hidden=!1;const r=Object.fromEntries(new FormData(a).entries()),e={name:t(r.name||"",80),email:t(r.email||"",120),phone:t(r.phone||"",20),phoneCountry:t(r.phoneCountry||"+33",6),company:t(r.company||"",120),projectType:t(r.projectType||"",30),message:t(r.message||"",3e3)};e.phone&&(e.phone=`${e.phoneCountry} ${e.phone}`);try{const h={access_key:"b2d398de-9e03-4bc2-a50e-60bc1af2da51",subject:`Nouvelle demande de contact · CapWeb (${e.projectType||"Non précisé"})`,from_name:e.name,email:e.email,phone:e.phone||"Non renseigné",company:e.company||"Non renseigné",project_type:e.projectType||"Non renseigné",message:e.message};if((await(await fetch("https://api.web3forms.com/submit",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(h)})).json()).success)v=Date.now(),a.reset(),m.hidden=!1,m.scrollIntoView({behavior:"smooth",block:"nearest"});else throw new Error("server error")}catch{alert("Une erreur est survenue. Merci de réessayer ou de nous contacter directement.")}finally{u.disabled=!1,p.hidden=!1,l.hidden=!0}});
